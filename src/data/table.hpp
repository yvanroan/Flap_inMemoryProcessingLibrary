...
class Table{
    ...
    private: 
        std::unordered_map<std::string, Array<std::optional<ArrayType>>> table;
        size_t numColum;
        size_t numRow;
    ...
    public:
        Table(std::unordered_map<std::string, Array<std::optional<ArrayType>>> input);
        void appendCol(std::string name, Array<std::optional<ArrayType>> input);
        void appendRow(std::vector<std::optional<ArrayType>> entry);
        void rename(std::string oldName, std::string newName);
        std::optional<ArrayType> dataAt(std::string name, size_t idx);
        Array<std::optional<ArrayType>> column(std::string name);
        ...
        std::unordered_map<std::string, Array<std::optional<ArrayType>>> getTable();
        size_t getNumColumn();
        size_t getNumRow();
}
#endif
